    ...
    public class TcpCommunicationService : BaseCommunicationService
    {
        ...
        public override bool Connect()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    logService.Info($"已成功连接到服务器: {_serverAddress}:{_serverPort}");
                ...
                {
                    _isConnected = false;
                    logService.Info($"连接服务器失败: {_serverAddress}:{_serverPort}{ex}");

                    ConnectionFailed?.Invoke(
                        $"连接服务器失败: {_serverAddress}:{_serverPort}", ex);
                    // 通知HL7Helper连接失败
                    //HL7Helper.Instance.NotifyConnectionState(
                    //    false,
        ...
        public override void Disconnect()
        {
            lock (_lockObject)
            {
                ...
                {
                    logService.Info($"断开服务器连接时发生错误{ex}");
                }
            }
        }
        ...
        private void CheckConnectionStatus(object state)
        {
            ...
            try
            {
                ...
                if (!_isConnected)
                {
                    ...
                    logService.Info("检测到TCP连接已恢复");
                    _isConnected = true;
                }
            }
            catch (Exception ex)
            {
                logService.Info($"检查TCP连接状态时发生错误{ex}");
        ...
        private void HandleDisconnection(string reason)
        {
            if (_isConnected)
            {
                logService.Info($"检测到服务器连接已断开: {reason}");
        ...
        private async Task ReceiveMessagesLoop(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested && _isConnected)
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                logService.Info("接收消息超过30秒未完成，重置接收状态");
                                startFound = false;
                    ...
                    if (endFound)
                    {
                        try
                        {
                            ...
                            logService.Info($"接收到HL7响应: {response}");
                        ...
                        {
                            logService.Info($"处理接收到的HL7消息时出错{ex}");
                        }
                    }
                }
                ...
                {
                    // 操作被取消，退出循环
                    logService.Info($"操作被取消，退出循环{ex}");
                    break;
                }
                catch (Exception ex)
                {
                    logService.Info($"接收HL7消息循环中发生错误{ex}");
                    ...
                    {
                        logService.Info($"操作被取消，退出循环2{ex2}");
                        break;
                    }
                }
            }
