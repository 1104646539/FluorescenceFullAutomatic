...
namespace FluorescenceFullAutomatic.UploadModule.Upload
{
    ...
    public class SerialCommunicationService : BaseCommunicationService
    {
        ...
        /// 构造函数
        /// </summary>
        public SerialCommunicationService(ILogService logService) : base(logService)
        {
            // 从配置中获取串口参数
        ...
        public override bool Connect()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    ConnectionSucceeded?.Invoke(true, $"已成功连接到串口: {_portName}");
                    logService.Info($"已成功连接到串口: {_portName}");
                    return true;
                }
                catch (Exception ex)
                {
                    _isConnected = false;
                    ConnectionFailed?.Invoke( $"连接串口失败: {_portName} ", ex);
                    logService.Info($"连接串口失败: {_portName}{ex}");
                    return false;
                }
            }
        }
        ...
        public override void Disconnect()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    _isConnected = false;
                    logService.Info("已断开串口连接");
                }
                catch (Exception ex)
                {
                    logService.Info($"断开串口连接时发生错误{ex}");
                }
            }
        }
