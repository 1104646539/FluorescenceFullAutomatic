using System;
using FluorescenceFullAutomatic.Platform.Model;
using FluorescenceFullAutomatic.ViewModels;

namespace FluorescenceFullAutomatic.Platform.Services
{
    public interface IReactionAreaService
    {
        bool GetReactionAreaNext(out int y, out int x);
        void UpdateItem(int y, int x, Func<ReactionAreaItem, ReactionAreaItem> func);
        void Clear();

        event Func<ReactionAreaItem, bool> _dequeueCallback;
        void Enqueue(ReactionAreaItem item);
        void SetDequeueDuration(int durationSeconds);
        int Count();
        bool IsEmpty();
        bool IsFull();
    }
    public class ReactionAreaService : IReactionAreaService
    {
        private System.Collections.Generic.Queue<Model.ReactionAreaItem> _queue = new System.Collections.Generic.Queue<Model.ReactionAreaItem>();
        private System.Timers.Timer _timer;
        private int dequeueSeconds = 60;
        public event Func<Model.ReactionAreaItem, bool> _dequeueCallback;
        public int MaxCount = 30;

        public ReactionAreaService()
        {
            // 初始化定时器，每秒检查一次
            _timer = new System.Timers.Timer(1000);
            _timer.Elapsed += Timer_Elapsed;
            _timer.AutoReset = true;
            _timer.Enabled = true;
        }

        private void Timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            Dequeue();
        }

        public bool GetReactionAreaNext(out int y, out int x)
        {
            return ReactionAreaViewModel.Instance.GetReactionAreaNext(out y, out x);
        }

        public void UpdateItem(int y, int x, Func<Model.ReactionAreaItem, Model.ReactionAreaItem> func)
        {
            ReactionAreaViewModel.Instance.UpdateItem(y, x, func);
        }

        public void Clear()
        {
            ReactionAreaViewModel.Instance.Clear();
            _queue.Clear();
        }

        /// <summary>
        /// 出队操作
        /// </summary>
        /// <returns>是否成功出队</returns>
        public bool Dequeue()
        {
            if (_queue.Count == 0)
                return false;

            var item = _queue.Peek();
            long currentTime = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
            //Log.Information($"出队时间:{item.ReactionAreaY} {item.ReactionAreaX} {item.EnqueueTime} + {dequeueSeconds} （{item.EnqueueTime + dequeueSeconds}）<= {currentTime}");
            // 检查是否达到出队时间
            if (item.EnqueueTime + dequeueSeconds <= currentTime)
            {
                // 调用回调函数，判断是否成功出队
                if (_dequeueCallback != null && _dequeueCallback(item))
                {
                    Model.ReactionAreaItem item1 = _queue.Dequeue();
                    //Log.Information($"出队1:{item1.ReactionAreaY} {item1.ReactionAreaX} {item1.TestResult.Id}");
                    return true;
                }
            }

            return false;
        }

        /// <summary>
        /// 入队操作
        /// </summary>
        /// <param name="item">要入队的项目</param>
        public void Enqueue(Model.ReactionAreaItem item)
        {
            if (item == null)
                return;

            // 设置入队时间
            item.EnqueueTime = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
            _queue.Enqueue(item);
        }

        public void SetDequeueDuration(int durationSeconds)
        {
            this.dequeueSeconds = durationSeconds;
        }

        public int Count()
        {
            if (_queue == null) return 0;
            return _queue.Count;
        }

        public bool IsEmpty()
        {
            return _queue == null || _queue.Count == 0;
        }

        public bool IsFull()
        {
            return Count() >= MaxCount;
        }
    }
}
