            ...
            try
            {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(msaSegment))
                        {
                            ...
                            if (fields.Length > 1)
                            {
                                ...
                                {
                                    Log.Information($"收到消息 {originalMessageId} 的肯定确认");
                                }
                ...
                else if (message is QCK_Q02 qck)
                {
                    try
                    {
                        ...
                        if (status == "OK")
                        {
                            Log.Information($"查询样本信息成功，有数据返回，等待接收详细数据...{queryTag}");
                        ...
                        {
                            Log.Information($"查询样本信息成功，无数据返回");
                        }
                        else
                        {
                            Log.Warning($"查询样本信息，响应状态异常: {status}");
                        }
                        ...
                        {
                            Log.Warning($"收到ID为 {queryTag} 的QCK_Q02，但没有找到对应的等待任务");
                        }
                    }
                    catch (Exception ex)
                    {
                        Log.Error(ex, "处理QCK_Q02消息时出错");
                    }
                }
                else if (message is DSR_Q03 dsr)
                {
                    try
                    {
                        ...
                        {
                            Log.Error(ex, "发送DSR_Q03的ACK响应时出错");
                        }
                        ...
                        )
                        {
                            ...
                            {
                                Log.Error(ex, "检查DSR_Q03消息的DSC段是否有更多数据时出错");
                            }
                        ...
                        try
                        {
                            ...
                            )
                            {
                                ...
                                while (!queryState.IsComplete)
                                {
                                    ...
                                    {
                                        Log.Warning($"等待样本信息完整接收超时，已接收 {queryState.DsrMessages.Count} 条消息，但不返回不完整数据");
                                        return new QueryResult(
            ...
            try
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        Log.Warning($"上传样本 {testResult.Barcode} 结果，{errorMsg}");
                        return new UploadResult(
                            UploadResultType.Failed,
                            testResult.Id,
                            errorMsg,
                ...
                catch (TimeoutException)
                {
                    ...
                    Log.Error($"上传样本 {testResult.Barcode} 结果时{errorMsg}");
                    return new UploadResult(
        ...
        )
        {
            ...
            while (retryCount < maxRetries)
            {
                try
                {
                    ...
                    Log.Information($"发送HL7消息 (ID={messageId}, 重试次数={retryCount + 1}/{maxRetries}): {encodedMessage}");
                    ...
                    using (var cts = new CancellationTokenSource(timeoutMilliseconds))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Warning($"等待消息 {messageId} 的响应超时，正在进行第 {retryCount + 1} 次重试");
                                continue;
                            }
                            else
                            {
                                Log.Error($"等待消息 {messageId} 的响应超时，已达到最大重试次数 {maxRetries}");
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex is TimeoutException)
                    {
                        ...
                        if (retryCount < maxRetries)
                        {
                            Log.Warning($"发送消息 {messageId} 时发生超时，正在进行第 {retryCount + 1} 次重试");
                            continue;
                        }
                    }

                    Log.Error(ex, $"发送消息 {messageId} 时发生错误");
                    throw;
                }
            }
